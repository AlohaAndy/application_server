...
public class PlusOneClient implements MessageTypes{
    ...
    public PlusOneClient(String _host, int _port) {
        try {
            //properties = new PropertyHandler(serverPropertiesFile);
            host = _host; //properties.getProperty("HOST");
            System.out.println("[PlusOneClient.PlusOneClient] Host: " + host);
            port = _port; //Integer.parseInt(properties.getProperty("PORT"));
    ...
    public static void main(String[] args) {
        ...
        if(args.length == 2) {
            client = new PlusOneClient(args[0], Integer.parseInt(args[1]));
        } else {
            client = new PlusOneClient("localhost", 2125);
        }
        client.run();
    }  
}
